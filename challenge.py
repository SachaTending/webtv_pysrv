...
from srv.challenge import WTVNetworkSecurity
from base64 import b64decode

def bdec(a: str) -> bytes:
    return b64decode(a.encode())

def gen_challenge(initial_key: str):
    initial_key = bdec(initial_key)
...
def check_challenge(ch: str, initial_key: str, cr: str):
    wsec = WTVNetworkSecurity(initial_key)
    p = wsec.ProcessChallenge(ch)
    return cr == ch

def check_challenge_old(ch: bytes, initial_key: bytes, cr: bytes):
